...
Partial Class workinghours
    ...
    Public Function GetAllEmployeeName() As String

        Dim conn As New Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionString").ToString)

        conn.Open()

        Dim selectString = "SELECT  DISTINCT s.StaffID,CONCAT(s.FirstName,'',s.LastName) as Name FROM [StaffWorkingHours] swh Inner Join Staff s On swh.StaffID = s.StaffID"
        ...
        While reader.Read()
            ...
            users.Add(reader("Name").ToString() + "," + reader("StaffID").ToString())
        ...
        For Each user As String In users
            Dim userArray = user.Split(",")

            outputString += "<tr>"
            outputString += "<td>" + userArray(0) + "</td>"
            outputString += GetTimeSlotHere(userArray(1))
            outputString += "<tr>"
        Next
    ...
    Private Function GetTimeSlotHere(id As String) As String

        Dim lbl1 As String = String.Empty
        Dim lbl2 As String = String.Empty
        Dim lbl3 As String = String.Empty
        Dim lbl4 As String = String.Empty
        Dim lbl5 As String = String.Empty
        Dim lbl6 As String = String.Empty
        Dim lbl7 As String = String.Empty
        ...
        Dim selectString = "SELECT StaffWorkingHoursId, s.StaffID, (FirstName + ' ' + LastName) Name, Format(StartTime,'hh:mm tt') StartTime, Format(EndTime,'hh:mm tt') EndTime, Format(StartTime,'MM/dd/yyyy') Headingdate, Day, Repeats FROM [StaffWorkingHours] swh Inner Join Staff s On swh.StaffID = s.StaffID Where s.StaffID=@s"

        Dim cmd As SqlCommand = New SqlCommand(selectString, conn)

        cmd.Parameters.Add("@s", Data.SqlDbType.BigInt).Value = id
        Dim reader As Data.SqlClient.SqlDataReader = cmd.ExecuteReader()
        ...
        While reader.Read()
            ...
            Dim getDate As DateTime = reader("Headingdate").ToString()

            If (Label1.Text).Equals(getDate.ToString("ddd, dd MMM")) Then
                lbl1 += reader("StartTime").ToString() + "-" + reader("EndTime").ToString()
                lbl1 += "<br/>"
            Else
                lbl1 += "<i class=""mdi mdi-plus-circle-outline""></i>"
            End If


            If Label2.Text.Equals(getDate.ToString("ddd, dd MMM")) Then
                lbl2 += reader("StartTime").ToString() + "-" + reader("EndTime").ToString()
                lbl2 += "<br/>"
            Else
                lbl2 += "<i class=""mdi mdi-plus-circle-outline""></i>"
            End If


            If Label3.Text.Equals(getDate.ToString("ddd, dd MMM")) Then
                lbl3 += reader("StartTime").ToString() + "-" + reader("EndTime").ToString()
                lbl3 += "<br/>"
            Else
                lbl3 += "<i class=""mdi mdi-plus-circle-outline""></i>"
            End If

            If Label4.Text.Equals(getDate.ToString("ddd, dd MMM")) Then
                lbl4 += reader("StartTime").ToString() + "-" + reader("EndTime").ToString()
                lbl4 += "<br/>"
            Else
                lbl4 += "<i class=""mdi mdi-plus-circle-outline""></i>"
            End If


            If Label5.Text.Equals(getDate.ToString("ddd, dd MMM")) Then
                lbl5 += reader("StartTime").ToString() + "-" + reader("EndTime").ToString()
                lbl5 += "<br/>"
            Else
                lbl5 += "<i class=""mdi mdi-plus-circle-outline""></i>"
            End If

            If Label6.Text.Equals(getDate.ToString("ddd, dd MMM")) Then
                lbl6 += reader("StartTime").ToString() + "-" + reader("EndTime").ToString()
                lbl6 += "<br/>"
            End If

            If Label7.Text.Equals(getDate.ToString("ddd, dd MMM")) Then
                lbl7 += reader("StartTime").ToString() + "-" + reader("EndTime").ToString()
                lbl7 += "<br/>"
            Else
                lbl7 += "<i class=""mdi mdi-plus-circle-outline""></i>"
            End If
        ...
        Dim staffworkinghrs = "<td><a href=""#"" class=""assignedslot"" data-toggle=""modal"" data-target=""#slotmodal"" "">" & lbl1 & " </a></td>" &
               "<td> <a href=""#"" class=""assignedslot"" data-toggle=""modal"" data-target=""#slotmodal"" "">" & lbl2 & " </a></td>" &
               "<td> <a href=""#"" class=""assignedslot"" data-toggle=""modal"" data-target=""#slotmodal"" "">" & lbl3 & " </a></td>" &
               "<td> <a href=""#"" class=""assignedslot"" data-toggle=""modal"" data-target=""#slotmodal"" "">" & lbl4 & " </a></td>" &
               "<td> <a href=""#"" class=""assignedslot"" data-toggle=""modal"" data-target=""#slotmodal"" "">" & lbl5 & " </a></td>" &
               "<td> <a href=""#"" class=""assignedslot"" data-toggle=""modal"" data-target=""#slotmodal"" "">" & lbl6 & " </a></td>" &
               "<td> <a href=""#"" class=""assignedslot"" data-toggle=""modal"" data-target=""#slotmodal"" "">" & lbl7 & " </a></td>"

        Return staffworkinghrs
    End Function
